...
threshold4 = 1
CountUnderThresh = sum(abs(FRAG1_ddH2O_FullDHT2{n}(:)) < threshold4)
filtered1_1000_ = FRAG1_ddH2O_FullDHT2{n};
for p = 1:dimension
    for q = 1: col
        if abs(FRAG1_ddH2O_FullDHT2{n}(p)) < threshold4
            filtered1_1000_(p) = 0;
...
filterInv1_1000 = InverseHaarTransform(filtered1_1000_,Matrices,n);
...
[ddH2OFit,ddH2OTau]= Anishkin_fit(ddH2O,0.0555,[0 .5 0 .5 .02]);
ddH2O2ExpFit =exp2Fit(ddH2O);

%% Write to Excel
dest = 'C:\Users\sukharevlab\Documents\Schams\Stopped Flow\All Data\Anishkin Fit Files\Tau\08-26-16 003 Constants Haar Transform.xlsx';
...
%% 
hold on
plot(time,FRAG1_ddH2O,time,filterInv1)
title('N = 10, threshold < 0.001 (Data Set 2)')
xlabel('Time (s)')
ylabel('Cell Scattering (V)')
axis([ 0 0.512 0.2 .7])

figure
plot(time,FRAG1_ddH2O,time,filterInv4)
title('N = 10, threshold < 0.004 (Data Set 2)')
xlabel('Time (s)')
ylabel('Cell Scattering (V)')
axis([ 0 0.512 0.2 .7])

figure
plot(time,FRAG1_ddH2O,time,filterInv100)
title('N = 10, threshold < 0.1 (Data Set 2)')
xlabel('Time (s)')
ylabel('Cell Scattering (V)')
axis([ 0 0.512 0.2 .7])
...
figure
plot(time,FRAG1_ddH2O,time,filterInv1_200)
xlabel('Time (s)')
ylabel('Scattering (V)')
title('N = 1, threshold < 0.2 (Data Set 2)')

figure
plot(time,FRAG1_ddH2O,time,filterInv5_200)
xlabel('Time (s)')
ylabel('Scattering (V)')
title('N = 5, threshold < 0.2 (Data Set 2)')

figure
plot(time,FRAG1_ddH2O,time,filterInv200)
xlabel('Time (s)')
ylabel('Scattering (V)')
title('N = 10, threshold < 0.2 (Data Set 2)')